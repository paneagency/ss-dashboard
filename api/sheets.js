  ...
  try {
    ...
    if (req.method === 'GET') {
      const [vendResp, provResp] = await Promise.all([
        sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: 'Vendedores!A:G' }),
      ...
      const vendors = vendRows.slice(vendStart)
        .filter(r => r[0]?.trim())
        .map(r => ({ name: r[0].trim(), commission: parseFloat(r[1]) || 0, email: r[2]?.trim() || '', direccion: r[3]?.trim() || '', taxId: r[4]?.trim() || '', notas: r[5]?.trim() || '', nombreFiscal: r[6]?.trim() || '' }));
    ...
    if (req.method === 'PUT' && req.body.mode === 'vendedor') {
      const { nombre, commission, email, direccion, taxId, notas, nombreFiscal } = req.body;
      ...
      await sheets.spreadsheets.values.update({
        spreadsheetId: SPREADSHEET_ID,
        range: `Vendedores!A${rowNum}:G${rowNum}`,
        valueInputOption: 'USER_ENTERED',
        requestBody: { values: [[nombre, commission ?? '', email || '', direccion || '', taxId || '', notas || '', nombreFiscal || '']] },
      });
