  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (mode === 'representantes') {
        try {
          const resp = await sheets.spreadsheets.values.get({ spreadsheetId: SPREADSHEET_ID, range: `${REPRESENTANTES_SHEET}!A:H` });
          const rows = (resp.data.values || []).slice(1);
          return res.json({ representantes: rows.map((r, i) => ({ row: i + 2, nombre: r[0] || '', email: r[1] || '', direccion: r[2] || '', taxId: r[3] || '', notas: r[4] || '', nombreFiscal: r[5] || '', autoFactura: r[6] === '1', facturarA: r[7] === '1' })).filter(r => r.nombre) });
    ...
    if (req.method === 'POST' && req.body.mode === 'representante') {
      const { nombre, email, direccion, taxId, notas, nombreFiscal, autoFactura, facturarA } = req.body;
      if (!nombre) return res.status(400).json({ error: 'nombre requerido' });
      await sheets.spreadsheets.values.append({
        spreadsheetId: SPREADSHEET_ID,
        range: `${REPRESENTANTES_SHEET}!A:H`,
        valueInputOption: 'USER_ENTERED',
    ...
    if (req.method === 'PUT' && req.body.mode === 'representante') {
      ...
      await sheets.spreadsheets.values.update({
        spreadsheetId: SPREADSHEET_ID,
        range: `${REPRESENTANTES_SHEET}!A${row}:H${row}`,
        valueInputOption: 'USER_ENTERED',
        requestBody: { values: [[nombre, email || '', direccion || '', taxId || '', notas || '', nombreFiscal || '', autoFactura ? '1' : '', facturarA ? '1' : '']] },
      });
